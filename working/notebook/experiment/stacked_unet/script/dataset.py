...
def read_image_label_mask(stack_path, cfg, preprocess, fragment_i, split_i, label_prefix):
    ...
    image_stack = preprocess(image_stack, fragment_i, split_i)

    label = cv2.imread(f"/kaggle/working/dataset_train/{label_prefix}_{fragment_i}_{split_i}.png", 0)
...
def visualize_train_images(id, train_images, train_labels, train_masks):
    ...
    channels = train_images[0].shape[2]
    fig_row = channels//3+1
    plt.figure(figsize=(10, fig_row*3))
...
def generate_valid_label(valid_fragment_i, valid_split_i, cfg, label_prefix):
    valid_label_gt = cv2.imread(f"{cfg.dataset_path}/{label_prefix}_{valid_fragment_i}_{valid_split_i}.png", 0)
...
class CustomDatasetVal(Dataset):
    def __init__(self, images, xyxy, cfg, labels=None, transform=None):
        self.images = images
        self.xyxy = xyxy
        self.cfg = cfg
        self.labels = labels
...
def generate_dataloader(train_images, train_labels,  valid_images, valid_labels, valid_xyxys, cfg):
    train_dataset = CustomDatasetTrain(train_images, cfg, labels=train_labels, transform=get_transforms(data='train', cfg=cfg))
    valid_dataset = CustomDatasetVal(valid_images, valid_xyxys, cfg, labels=valid_labels, transform=get_transforms(data='valid', cfg=cfg))

    train_loader = DataLoader(train_dataset, batch_size=cfg.train_batch_size, shuffle=True, num_workers=cfg.num_workers, pin_memory=True, drop_last=True,)
    valid_loader = DataLoader(valid_dataset, batch_size=cfg.valid_batch_size, shuffle=False, num_workers=cfg.num_workers, pin_memory=True, drop_last=False)
